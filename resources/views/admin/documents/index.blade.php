...
<div class="space-y-8">
    ...
    <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
        @foreach([
            ['label' => 'Menunggu Verifikasi', 'value' => $waitingDocuments, 'desc' => 'Perlu dicek admin', 'class' => 'text-yellow-700'],
            ['label' => 'Diterima', 'value' => $acceptedDocuments, 'desc' => 'Berkas valid', 'class' => 'text-green-700'],
            ['label' => 'Ditolak', 'value' => $rejectedDocuments, 'desc' => 'Perlu upload ulang', 'class' => 'text-red-700'],
            ['label' => 'Belum Upload', 'value' => $notUploadedDocuments, 'desc' => 'Belum ada file', 'class' => 'text-slate-700'],
        ] as $card)
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="border-b border-slate-200 pb-5">
            ...
            <p class="mt-2 text-sm leading-6 text-slate-600">
                Cari berdasarkan nama camaba, nomor pendaftaran, NIK, atau nama file.
            </p>
        </div>

        <form action="{{ route('admin.documents.index') }}" method="GET" class="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-5">
            <div class="xl:col-span-2">
                <label class="text-sm font-bold text-slate-700">Pencarian</label>
                <input type="text"
                       name="keyword"
                       value="{{ request('keyword') }}"
                       placeholder="Nama, no pendaftaran, NIK, file..."
    ...
    <div class="rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-col justify-between gap-4 border-b border-slate-200 p-6 md:flex-row md:items-center">
            <div>
                ...
                <p class="mt-2 text-sm leading-6 text-slate-600">
                    Data berkas diambil dari tabel applicant_documents.
                </p>
            </div>
                    ...
                    @forelse($documents as $document)
                        <tr class="transition hover:bg-slate-50">
                            ...
                            <td class="px-6 py-5">
                                ...
                                <p class="mt-1 text-xs text-slate-500">
                                    {{ $document->documentType?->is_required ? 'Wajib' : 'Opsional' }}
                                </p>
                            </td>
                            ...
                            <td class="px-6 py-5">
                                <div class="flex justify-end gap-2">
                                    ...
                                    @if($document->file_path)
                                        <button type="button"
                                                onclick="Swal.fire({
                                                    ...
                                                    text: 'Preview file akan kita sambungkan setelah storage upload aktif. File path: {{ $document->file_path }}',
                                                    icon: 'info',
                                    ...
                                    @if($document->status !== 'diterima')
                                        <form action="{{ route('admin.documents.accept', $document) }}" method="POST"
